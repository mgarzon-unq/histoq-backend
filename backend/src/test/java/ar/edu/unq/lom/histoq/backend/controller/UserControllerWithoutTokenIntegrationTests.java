...
public class UserControllerWithoutTokenIntegrationTests {
    @Autowired
    private MockMvc mvc;
    ...
    @Test
    void findNonExistentUserReturnsHttpErrorNotFound() throws Exception {
        mvc.perform(get("/users/pepe@grillo"))
                .andExpect(status().isNotFound());
    }

    @Test
    void requestWithoutAdminReturnsHttpErrorForbidden() throws Exception {
        Mockito.doThrow(UnauthorizedAccessException.class).when(this.securityService).adminAccessControl();

        mvc.perform(get("/users/registration-requests"))
